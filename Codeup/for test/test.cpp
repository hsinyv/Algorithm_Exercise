...
int main() {
	ifstream in("day.txt");
	ofstream cal("temp.txt");
	int d; string m; string y;
	while (in >> d >> m >> y) {
		int date[6]; bool flag;
		get_date(d,m,y,date, flag);
		string x = cal_day(date, flag);
		int a;
		if(x == "Monday") a = 1;
		else if(x == "Tuesday") a = 2;
		else if(x == "Wednesday") a = 3;
		else if(x == "Thursday") a = 4;
		else if(x == "Friday") a = 5;
		else if(x == "Saturday") a = 6;
		else if(x =="Sunday") a = 7;
		else a = -1;
		cal << d << " " << m << " " << y << " " << x << " " << a << endl;
	}
	in.close();
